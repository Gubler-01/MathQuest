...
sphere_pos3 = [5 ,0, 0]  # Posición inicial de la segunda esfera
...
def options_3d():
    ...
    gluLookAt(0, -15, 0, 0, 0, 0, 0, 0, 1)  # Ajusta la posición de la cámara
        ...
        if not paused:
            ...
            if show_figures:
             ...
             # Dibuja la figura en la nueva posición
               Dibujar.draw_cube((x, y, 0), (1, 1, 1))
               Dibujar.draw_arm((x-0.7, y, -2.6), (0.2, 0.2, 1.0), (30, 0.0, 0.0, 1.0))
               Dibujar.draw_body((x, y, -2.5), (1, 1, 1), (1, 1, 1))
               Dibujar.draw_arm2((x+0.7, y, -2.6), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               Dibujar.draw_pie((x+0.5, y, -5), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               Dibujar.draw_pe2((x-0.5, y, -5), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               Dibujar.draw_eyes1(((x+(-0.9))+x, y+(-1.3), 2.0), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               Dibujar.draw_eyes2(((x+1.00)+x, -1.3, 2.0), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               Dibujar.draw_mouth(((x+(-0.01))+x/2, -0.7, 0.2), (1, 1, 1), (0.0, 0.0, 1.0, 1.0))
               #fincreper
            ...
            if key_1_pressed:
                ...
                esfera2_pos = (5.1, 0, 0)
                esfera2_radio = 1.04
                # Calcula la distancia entre las esferas
                distance = ((draw_cube[0] - esfera2_pos[0]) ** 2 + (draw_cube[1] - esfera2_pos[1]) ** 2 + (draw_cube[2] - esfera2_pos[2]) ** 2) ** 0.5
                if (esfera2_radio + esfera2_radio >= distance):
            ...
            if key_2_pressed:
                render_text("pokemon",1.5, 2.0)  # Personaliza los valores según tu función
                render_text("letras: b,n,m",6.5, -4.0)
                if show_figure_b:
                    glColor4f(1.0, 1.0, 0.0, 1.0)
                    pinatmos.ojo1((-0.2 - translate_x, -1, 0.3), (.2, .1, .2))
                    pinatmos.ojo2((0.4 - translate_x, -1, 0.3), (.2, .1, .2))
            ...
            render_text("Para elegir escoja de acuerdo al numero", -17.5, 2.0)            # Mover arriba a 1.2
            ...
            if selected_number is not None:
                ...
                gluSphere(sphere, 1.0, 32, 16)
                glPopMatrix()
                render_text("Cuanto es 2*5+3-4", -11.5, 2.0)            # Mover arriba a 1.2
                render_text(f"Seleccionado: {selected_number}", -2.5, -6.0)  # Personaliza los valores según tu función
